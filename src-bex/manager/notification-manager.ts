...
export default class NotificationManager extends EventEmitter {
  ...
  async showPopup (setCurrentPopupId: () => void, currentPopupId: number) {
    ...
    if (popup) {
      ...
      await this.platform.focusWindow(popup.id as number)
    } else {
      ...
      try {
        ...
        left = Math.max(
          lastFocused.left as number + (lastFocused.width as number - NOTIFICATION_WIDTH),
          0
        )
      } catch (_) {
      ...
      if (popupWindow.left !== left && popupWindow.state !== 'fullscreen') {
        await this.platform.updateWindowPosition(popupWindow.id as number, left, top)
      }
  ...
  /**
     ...
     */
  _getPopupIn (windows: Windows.Window[]) {
    return windows
      ? windows.find((win) => {
