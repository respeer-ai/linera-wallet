...
export class Application {
  static microchainApplications = async (microchain: string): Promise<ApplicationOverview[]> => {
  ...
  static applications = async (microchains: string[]) => {
    ...
    for (const microchain of microchains) {
      _applications.push(...await Application.microchainApplications(microchain))
    }
