...
export class ApplicationOperation {
  static existChainApplication = async (chainId: string, applicationId: string): Promise<boolean> => {
  ...
  static queryApplication = async (chainId: string, applicationId: string, query: DocumentNode, operationName: string, variables?: Record<string, unknown>): Promise<Uint8Array | undefined> => {
    const network = await dbBridge.Network.selected() as db.Network
    if (!network) return
  ...
  static subscribeCreatorChain = async (chainId: string, applicationId: string, applicationType: db.ApplicationType, force?: boolean): Promise<boolean> => {
    if (!force && await dbBridge.ApplicationCreatorChainSubscription.subscribed(chainId, applicationId)) return true

    if (await dbBridge.ChainOperation.exists(chainId, db.OperationType.SUBSCRIBE_CREATOR_CHAIN, applicationId))
      return true

    try {
      const queryRespBytes = await ApplicationOperation.queryApplication(chainId, applicationId, SUBSCRIBE_CREATOR_CHAIN, 'subscribeCreatorChain')
      const operation = {
  ...
  static requestSubscribe = async (chainId: string, applicationId: string) => {
    const queryRespBytes = await ApplicationOperation.queryApplication(chainId, applicationId, LEGACY_REQUEST_SUBSCRIBE, 'requestSubscribe')
