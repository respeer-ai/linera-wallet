...
export class Microchain {
  ...
  static initMicrochainStore = async (keyPair: Ed25519SigningKey, chainId: string, messageId: string, certificateHash: string) => {
  ...
  static openMicrochain = async (): Promise<db.Microchain> => {
    ...
    await Microchain.initMicrochainStore(keyPair, resp.chainId as string, resp.messageId as string, resp.certificateHash as string)
    // The first block will be signed in BlockView

    const microchain = await dbBridge.Microchain.create(owner.owner, resp.chainId as string, resp.messageId as string, resp.certificateHash as string)

    return microchain
  }

  static importMicrochain = async (chainId: string, messageId: string, certificateHash: string): Promise<db.Microchain> => {
    const owner = (await dbWallet.owners.toArray()).find((el) => el.selected)
    if (!owner) return Promise.reject(new Error('Invalid owner'))
    ...
    await Microchain.initMicrochainStore(keyPair, chainId, messageId, certificateHash)
    // The first block will be signed in BlockView

    const microchain = await dbBridge.Microchain.create(owner.owner, chainId, messageId, certificateHash)
