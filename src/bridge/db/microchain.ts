...
export class Microchain {
  static microchains = async (offset: number, limit: number, imported?: boolean): Promise<db.Microchain[]> => {
    return (await dbWallet.microchains.offset(offset).limit(limit).toArray()).filter((el) => !imported || el.imported)
  }

  static microchain = async (microchain: string): Promise<db.Microchain | undefined> => {
  ...
  static ownerMicrochains = async (offset: number, limit: number, owner: string, imported?: boolean): Promise<db.Microchain[]> => {
  ...
  static microchainOwner = async (microchain: string): Promise<db.Owner | undefined> => {
  ...
  static create = async (owner: string, microchainId: string, messageId?: string, certificateHash?: string, name?: string, _default?: boolean): Promise<db.Microchain> => {
