...
export class Activity {
  static addressActivities = async (publicKey: string): Promise<db.Activity[]> => {
  ...
  static ownerActivities = async (offset: number, limit: number, owner: db.Owner): Promise<db.Activity[]> => {
    const microchainOwners = await MicrochainOwner.ownerMicrochainOwners(owner.owner) || [] as db.MicrochainOwner[]
  ...
  static create = async (
    ...
    const activities = (await dbWallet.activities.toArray()).filter((activity) => {
      return activity.sourceChain === sourceChain &&
